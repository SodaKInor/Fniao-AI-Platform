<template >
  <a-card :body-style='{padding:0}' class="j-address-list-right-card-box" style="padding: 0px !important;"  :bordered="false">
    <div class="ceshi" style="width: 100%;height:50px;"> <input type="text" v-model="url">
      <button @click="geturl()">播放4</button>
      <button @click="closeurl()">销毁4</button>
    </div>
    </div>

    <div class="buttons-box4" id="buttonsBox4">
  ...
  export default {
    ...
    data() {
      return {
        ...
        positionInfo: {},
        _uid: 0,
        url: window._CONFIG.VUE_APP_VIDEO_STREAM_URL || '',
    ...
    methods: {
      ...
      initPro() {
        let options = {
          container: document.getElementById('buttonsBox4'),
      ...
      geturl() {
        ...
        jessibucaPlayer[this._uid].play(this.url).then(() => {
          console.log('play success')
             jessibucaPlayer[this._uid].on("start", function () {
              console.log('start render')
          
              // 如果有截图需求。需要延迟一下，因为代码中无法感应到画面被渲染出来了。
              // 1s 之后截图
              const canvas = document.getElementById('buttonsBox4').getElementsByTagName('canvas')[0];
               console.log('canvas',canvas)
              // const ctx = canvas.getContext('2d');
                 // canvas.style.width = '800px'; // 设置 Canvas 的宽度
                 // canvas.style.height = '850px'; // 设置 Canvas 的高度
          //      canvas.style.transform = 'rotate(45deg)'; // 旋转 Canvas
          canvas.style.transform = 'scale(1)';
                canvas.style.overflow = 'hidden'; // 旋转 Canvas
     
          })
          // 在 mounted 钩子中获取 Canvas 元素并操作
           
            // #buttonsBox4 canvas{
            //    width: 800px; /* 设置 Canvas 容器的宽度 */
            //    height: 850px; /* 设置 Canvas 容器的高度 */
            //    overflow: hidden; /* 隐藏 Canvas 超出容器部分 */
            //    transform: rotate(45deg); /* 旋转 Canvas */
            // }
            // // 设置 Canvas 的宽度和高度（拉伸）
            // canvas.width = 2800;
            // canvas.height = 2800;
...
<style scoped>
 ...
 .ant-card-body {
  ...
  canvas{
      width: 200px; /* 设置 Canvas 的宽度 */
      height: 150px; /* 设置 Canvas 的高度 */
      transform-origin: center; /* 设置旋转的原点为 Canvas 中心 */
