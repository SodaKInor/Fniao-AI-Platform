...
4. 旧本地算法：按引用清单删除无调用者执行代码；历史表、实体、CRUD 和通用播放器保留。
5. 可复用依赖：根据用户后续将在新目录重构的决定，原生算法依赖、私有 JAR 构建脚本和播放器制品
   保留为预留能力，但不得重新接入本轮已退役入口。

机器可重复检查入口：`scripts/check-backend-modules.py`、`scripts/check-frontend-modules.py` 和
`scripts/check-legacy-execution.py`。JSON 输出记录文件数、依赖边、循环、退役入口与明确保留项。
