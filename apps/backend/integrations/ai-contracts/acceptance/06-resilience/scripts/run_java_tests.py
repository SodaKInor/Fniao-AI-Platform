#!/usr/bin/env python3
"""Compile on Java 8 and run against a disposable database without touching saved history."""
import hashlib
import json
...
WORK = ROOT.parent / 'drafts/validation/java'
PARALLEL = ROOT.parent.parent
LIBS = PARALLEL / '00-integration/drafts/round3/java/libs'
ENV = PARALLEL / '00-integration/drafts/round3/tests.env'
MYSQL = 'wgai-ri-00-integration-mysql-1'
NETWORK = 'wgai-ri-00-integration_network'


def sql(statement, database=''):
    assert not database or re.fullmatch(r'ai_06_verify_[a-f0-9]+', database)
...
def main():
    assert ROOT.name == 'code' and ROOT.parent.name == '06-resilience'
    ...
    database = 'ai_06_verify_' + uuid.uuid4().hex[:12]
    ...
    command = ['docker', 'run', '--rm', '--network', NETWORK,
               ...
               '--env-file', str(ENV), '-v', str(ROOT) + ':/workspace:ro',
               '-v', str(WORK) + ':/validation', '-v', str(LIBS) + ':/validation/lib:ro',
               'maven:3.8.8-eclipse-temurin-8', 'sh',
               '/workspace/backend-github/integrations/ai-contracts/acceptance/06-resilience/scripts/run-java8.sh']
    result = None
    try:
    ...
    receipt = {
        ...
        'javaClassMajor': 52,
        'database': '06-owned disposable schema; dropped after test',
        'sourceReadOnly': True,
