...
public class TabAiBaseController extends JeecgController<TabAiBase, ITabAiBaseService> {
	...
	// @GetMapping(value = "/test")
	 public void test() {
		 String opencvNativePath = System.getenv().getOrDefault("OPENCV_NATIVE_PATH", "");
		 String rtspUrl = System.getenv().getOrDefault("WGAI_CAMERA_STREAM_URL", "");
		 if (opencvNativePath.isEmpty() || rtspUrl.isEmpty()) {
			 log.warn("OpenCV/camera test is disabled because its local contract is not configured");
			 return;
		 }
		 System.load(opencvNativePath);
