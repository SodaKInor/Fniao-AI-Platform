#!/usr/bin/env python3
"""Run Java 8 remote-AI tests against a disposable database and retain only the receipt."""
...
def sql(statement, database=""):
    assert not database or re.fullmatch(r"ai_07_verify_[a-f0-9]+", database)
    command = [
...
def main():
    assert ROOT.name == "code" and ROOT.parent.name == "07-cleanup"
    ...
    database = "ai_07_verify_" + uuid.uuid4().hex[:12]
    sql(
    ...
    command = [
        ...
        "/workspace/backend-github/integrations/ai-contracts/acceptance/07-cleanup/scripts/run-java8.sh",
    ]
    result = None
    try:
    ...
    receipt = {
        ...
        "javaClassMajor": 52,
        "database": "07-owned disposable schema; dropped after test",
        "sourceReadOnly": True,
        "log": str(WORK / "tests.log"),
        "logSha256": hashlib.sha256((WORK / "tests.log").read_bytes()).hexdigest(),
    }
    (OUT / "group3-java8-tests.actual.json").write_text(json.dumps(receipt, indent=2) + "\n")
