...
const rows = ['src/api/ai', 'src/services/ai', 'src/components/ai', 'src/views/ai'].flatMap(dir => files(path.join(frontend, dir))).map(file => {
...
assert(/pass 16/.test(tests)); assert(!/\berror\b/.test(lint))
...
fs.writeFileSync(path.join(evidence, 'frontend-checks.json'), JSON.stringify({ status: 'PASS', tests: 16,
