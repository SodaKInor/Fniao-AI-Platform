...
def main():
    rows = []
    schemas = json.loads((CONTRACTS / 'v1/business.openapi.json').read_text())['components']['schemas']
    for path in sorted(AI.rglob('*.java')):
        ...
        layer = path.relative_to(AI).parts[0]
        ...
        if layer == 'api':
            allowed += ['com.fasterxml.jackson.annotation.',
                        'org.jeecg.modules.ai.domain.JobState', 'org.jeecg.modules.ai.domain.ErrorCode']
    ...
    for name, values in [('JobState', schemas['JobState']['enum']),
    ...
    (EVIDENCE / 'type-checks.json').write_text(json.dumps({
        ...
        'coverage': 'Java AST declarations/imports/physical method spans and DTO/schema mappings. '
