...
状态说明：第 8 批目录整合已经完成本地合并，以下施工路径描述保留为变更历史；真实 GPU 门禁仍开放。
...
- 最终仓库以当前 Git 根为唯一活动目录。第 8 批已按“串行 apps 壳 → 同源 database/remote-inference 零重叠工作树 → 串行合并”执行；临时工作树路径只保留在历史记录中。
