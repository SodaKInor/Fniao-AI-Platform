...
状态：**PASS / ROUND4_MOCK_GATE_RELEASED**。代码已按序合并，真实后端组合验收和真实页面联合验收均通过；本结论只放行第四轮模拟闭环，不代表 RTX 5070、上传视频或实时流已经联调。
...
本记录的统一验收提交完整 SHA 登记于代码树外的 00-integration/HANDOFF.md 和 WGAI-parallel/WORKSPACES.json，避免自身哈希循环。该提交是第四轮模拟闭环的唯一放行点；05/06/07/08 及新增视频能力仍按后续门禁执行。
...
| 3.4 | PASS | 四账号真实登录：owner_a/owner_b 有执行入口，viewer 页面显示无权限且上传/提交禁用，nomenu 无 AI 菜单且直接路由进入 404；匿名直达回登录页，owner_b 读取 owner_a 任务显示不存在或无权；旧入口显示停用说明，管理查询菜单保留 |
...
| 4.6 | PASS | 同交付源码、冻结响应契约、16 项页面/API/轮询/Blob 测试、完整前端构建与定向 lint；并由4.7真实浏览器闭环补足联合验收 |
| 4.7 | PASS | 用户在操作时授权验证码后，以真实18100页面完成上传、200/202、后台轮询、结构化结果、预览/下载、刷新详情/历史；截断为12字节时不发布预览并显示网络中断，恢复后同一79字节成果可预览和触发下载。详见 browser-e2e.json |

总表更新 12 个已验证任务；连同第一轮为 **23/41**。第四轮模拟闭环已形成统一验收提交并登记放行边界。05/06/07/08、真实 GPU、上传视频与实时流仍未完成，不因本次放行被提前标记或启动。
...
- safety-e2e.json：真实 HTTP 截断和 JSON 错误；browser-e2e.json 进一步记录真实页面在12字节断流时拒绝预览、恢复后读取完整79字节成果。
...
- menu-e2e.json：真实四账号菜单/权限响应，经实际前端导航模块转换后核对；browser-e2e.json 再以真实页面确认 nomenu 不得到 AI 路由、viewer 不能上传或提交、跨用户任务不可读。
