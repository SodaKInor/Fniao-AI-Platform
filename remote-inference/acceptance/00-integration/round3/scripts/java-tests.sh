...
if [ ! -f /validation/server.p12 ]; then
  keytool -genkeypair -alias fixture -keyalg RSA -keysize 2048 -storetype PKCS12 -keystore /validation/server.p12 -storepass fixture-only -keypass fixture-only -dname CN=fixture.invalid -ext SAN=ip:127.0.0.1 -validity 30
  keytool -exportcert -rfc -keystore /validation/server.p12 -storepass fixture-only -alias fixture -file /validation/server.crt
fi
...
java -Djava.io.tmpdir=/validation/tmp -Dlogback.configurationFile=/validation/logback-test.xml \
  ...
  org.jeecg.modules.ai.assetsjobs.PersistenceTest org.jeecg.modules.ai.assetsjobs.WorkflowTest \
  org.jeecg.modules.ai.assetsjobs.ApiTest org.jeecg.modules.ai.assetsjobs.StorageTest \
  org.jeecg.modules.ai.assetsjobs.ConfigurationTest org.jeecg.modules.ai.integration.CombinedConfigurationTest \
  org.jeecg.modules.ai.integration.ControllerProxyTest
